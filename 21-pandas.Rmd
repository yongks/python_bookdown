...
Take note that in below empty_df1 and empty_df2 refers to **same memory location**. Meaning they contain similar data.
...
**Basic DataFrame with default Row Label and Column Header**, which default to 0,1,2,... numbering.
...
pd.DataFrame(data)
```
**Specify Row Label and Column Header** with `columns` and `index` parameters.

```{python}
pd.DataFrame(data, 
  columns = ['empID','name','salary','year'],
  index   = ['r1','r2','r3'])
```
...
**Default Column Name Follow Dictionary Key**, and **index** is integer starting from 0.  
Note that missing data is assumed as NaN

**Specify Column  during Creation** can acts as column filter and manual arrangement. 

```{python}
data = [{"name":"Yong", "id":1,"zkey":101},{"name":"Gan",  "id":2   }]
...
## Filter Which Columns To Include
...
DataFrame(  data = {'column1': list1,
            ...
            columns  = column_list)
```
By default, DataFrame will **arrange the columns according to the dict keys order**, unless **columns** is specified. Index by default starts from 0 if not specified.

```{python}
## columns arrangement defaul to dict's keys
...
pd.DataFrame(data)
## Select What Columns To Show. Specify row label.
pd.DataFrame(data, columns=['name','salary','year'], index=data.get('empID'))
```
...
- **`.add()`**, has the same result as **`+`**. 
- However, when **`fill_value`** is specified, ONE-SIDED none matching cells is assumed to have value specified in **`fill_value`**. Only BOTH-SIDED non matching cells will results in NaN. 
...
Note that columns or rows in Series that are not found in DataFrame, it still be created as the result. This is similar behavior as operating DataFrame with Dataframe.
...
- Resetting index will resequence the index as 0,1,2 etc  
...
- Updating index **doesn't reorder** the data sequence. To reorder, use **`.reindex()`** instead.
...
```{python, results='hold'}
## before reindex
df.index = (200,201,202,203,204)
df
## after reindex, the order is re-arranged
df.reindex([203,202,201,204,200])
```
#### Rename Axis
...
```{python}
df.rename_axis('super_id', axis=0)
```
### Subsetting Rows
...
```{python}
new_columns = ['empID', 'year.1','salary','year.2','glamour']
...
- Support **inpalce** option.
- Specify only column that require name change.
- No matching columns **will not** return error. That is great.
...
`.reindex()` has both functions of **rearranging**  and **fileter** columns.
...
- **Non Matching** columns will result in **ERROR**  

```{python, results='hold', error=TRUE}
new_colorder = [ 'salary', 'year1', 'year2', 'baba']

## Both methods returns the same result
df.reindex(columns = new_colorder)  ## no Error, even though 'baba' doesn't exist
df[new_colorder]       ## Error necause 'baba' columns does not exist
df[new_colorder[:-1]]  ## Fixed, removed 'baba' from the column list
```
#### Create New Column
...
**Filter by `like` - Substring Matching**, always return DataFrame.
...
```{python}
df.filter( items=('year1','year2', 'name'),  axis=1)  ## or axis = 1
```
